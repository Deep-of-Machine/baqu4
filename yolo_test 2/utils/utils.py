...
def bgr2color_name(b, g, r):
    target = (b,g,r)
    red_dst = distance.euclidean(target, red)
    blue_dst = distance.euclidean(target, blue)
    yellow_dst = distance.euclidean(target, yellow)
    another_dst = distance.euclidean(target, another)

    least = min(red_dst, blue_dst, yellow_dst, another_dst)
    if least == red_dst:
        color = (0,0,255)
        label = 'red'
    if least == blue_dst:
...
lower_blue = (115-40, 10, 10) # hsv 이미지에서 바이너리 이미지로 생성 , 적당한 값 30
upper_blue = (115+40, 350, 350)

lower_red = (6-6, 10, 10) # hsv 이미지에서 바이너리 이미지로 생성 , 적당한 값 30
upper_red = (6+4, 350, 350)

lower_yellow = (19-6, 10, 10) # hsv 이미지에서 바이너리 이미지로 생성 , 적당한 값 30
upper_yellow = (19+30, 350, 350)
...
def plot_one_box(x, img, line_thickness=None):  # Plots one bounding box on image img
    ...
    elif g>0.3 and r>0.45:   #0.3~0.35 0.2 or 0.35 0.3
        label = 'yellow'
        color = (0, 255, 255)
    ...
    # b,g,r = colr
    # label, color = bgr2color_name(h,s,v)

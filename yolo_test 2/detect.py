...
def outer_control(x):
    ...
    if x < 45 or x > 135 :
        ...
        if outer - 15 < x < outer + 15 or count < 3:
            outer = x
            return x
        else:
            print('######### 상대 이상치 처리 ############')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', type=int, default=32*20, help='size of each image dimension') # 32*20             best 32* 20                 #32*13
    parser.add_argument('--conf-thres', type=float, default=0.07, help='object confidence threshold') #  0.1               best!!!! 0.01                       #0.5
    parser.add_argument('--nms-thres', type=float, default=0.001, help='iou threshold for non-maximum suppression') #  0.01           0.01         #0.45

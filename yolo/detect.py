...
def steering_theta(w1, w2):
    if np.abs(w1) > np.abs(w2):  # 우회전
        if w1 * w2 < 0:  #정방향 or 약간 틀어진 방향
            ...
            theta = matching(angle, -np.pi/2, np.pi/2, 90, 135)
        ...
        if w1 * w2 < 0:  #정방향 or 약간 틀어진 방향
            w1 = -w1
            angle = np.arctan(np.abs(math.tan(w1) - math.tan(w2)) / (1 + math.tan(w1)*math.tan(w2)))
            theta = matching(angle, -np.pi/2, np.pi/2, 45, 90)
        elif w1 * w2 > 0:  #극한으로 틀어진 방향
            if w1 > w2:
                theta = 90
            else:
    ...
    for i, (path, img, im0) in enumerate(dataloader):
        ...
        if len(pred) > 0:
            ...
            if np.isnan(left_calculated_weight)!=True and np.isnan(right_calculated_weight)!=True:
                print(steering_theta(left_calculated_weight, right_calculated_weight))

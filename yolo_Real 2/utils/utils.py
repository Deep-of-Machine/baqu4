...
def bgr2color_name(b, g, r):
    ...
    green_dst = distance.euclidean(target, green)

    least = min(red_dst, blue_dst, yellow_dst, green_dst)
    if least == red_dst:
        color = red
        label = 'red'
    if least == blue_dst:
...
def plot_one_box(x, img, line_thickness=None):  # Plots one bounding box on image img
    ...
    elif g>0.25 and r>0.3:   #0.3~0.35 0.2 or 0.35 0.3
        label = 'yellow'
        color = (0, 255, 255)
    elif g > 0.3:               #0.35
        label = 'green'
    ...
    '''
    b,g,r = colr
    label, color = bgr2color_name(b,g,r)

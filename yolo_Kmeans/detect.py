...
#     right_Y = []
#     for i in range(480):
#         left_X.append(pts_left[0][i][0])  # 왼쪽 x값 전체
#         left_Y.append(pts_left[0][i][1])  #
#         right_X.append(pts_right[0][i][0])
#         right_Y.append(pts_right[0][i][1])
#     # print(left_X)
...
#     return left_calculated_weight, right_calculated_weight
...
def detect(
    ...
    for i, (path, img, im0) in enumerate(dataloader):
        ...
        if len(pred) > 0:
            ...
            try:
                ...
                for i in range(len(kmeans.labels_)):
                    if kmeans.labels_[i] == 0:
                        xline.append(list(points[i]))
                        # print("x: ", points[i])
                    else:
                        yline.append(list(points[i]))
                        # print('y', points[i])
            ...
            #     if xline[i] < px[0]/2:
            #         left_x.append(xline[i])
            #         left_y.append(yline[i])
            #     else:
            #         right_x.append(xline[i])
            #         right_y.append(yline[i])

            ##### xline이 xline이 아닐 수 있음..
            ## 클러스터링 된 x 좌표 합한거 비교 -> 좌 우 차선 판단
            sumx = 0
            for i in range(len(xline)):
                sumx += xline[i][0]
            ...
            right = sumy/len(yline)

            
            if left > right:
                for i in range(len(xline)):
                    right_x.append(xline[i][0])
                    right_y.append(xline[i][1])

                for i in range(len(yline)):
                    left_x.append(yline[i][0])
                    left_y.append(yline[i][1])
            else:
                for i in range(len(xline)):
                    left_x.append(xline[i][0])
                    left_y.append(xline[i][1])

                for i in range(len(yline)):
                    right_x.append(yline[i][0])
                    right_y.append(yline[i][1])
                


            # print('0: ',left_x, left_y)
            # print('1: ', right_x, right_y)
            ...
            l_target = left_calculated_weight * 1920 + left_calculated_bias
            ...
            r_target = right_calculated_weight * 1920 + right_calculated_bias
            print(f"y = {right_calculated_weight} * X + {right_calculated_bias}")
            im0 = cv2.line(im0,(int(1920/2),1080),(int(1920/2),int(0)),(255,0,0),3)
            ...
            # 기울기 둘다 있을 때
            if np.isnan(left_calculated_bias)!=True and np.isnan(right_calculated_bias)!=True :
                im0 = cv2.line(im0,(0,int(left_calculated_bias)),(int(1920),int(l_target)),(0,0,0),10)
                im0 = cv2.line(im0,(int(0),int(right_calculated_bias)),(1920,int(r_target)),(0,0,0),10)
                cv2.circle(im0, (int(cross_x), int(cross_y)), 10, (0, 0, 255), -1, cv2.LINE_AA)


            

            # print(left_calculated_weight)
            # print(right_calculated_weight)

            if np.isnan(left_calculated_weight)!=True and np.isnan(right_calculated_weight)!=True:
            ...
            # lefty = px[1] - np.array(y_sorted_by_x[:idx+1])
            # rightx = sorted_x[idx+1:]
            # righty  = px[1] - np.array(y_sorted_by_x[idx+1:])
            ...
            # # print(sum([(left_X[i] - x_mean ) * ( left_Y[i] - y_mean) for i in range (len(left_X))]) / sum([math.pow(i - x_mean, 2) for i in left_X]))
            # # print(leftx, lefty, rightx, righty)
            ...
            #     print(steering(left_calculated_weight, right_calculated_weight))

            #     result = round(float(steering(left_calculated_weight, right_calculated_weight)), 3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', type=int, default=32*20, help='size of each image dimension') #32*13

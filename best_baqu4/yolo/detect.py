...
def detect(
    ...
    for i, (path, img, im0) in enumerate(dataloader):
        ...
        left_X = leftx
        left_Y = lefty
        x_mean = np.mean(left_X) 
        y_mean = np.mean(left_Y) 
        # print(sum([(left_X[i] - x_mean ) * ( left_Y[i] - y_mean) for i in range (len(left_X))]) / sum([math.pow(i - x_mean, 2) for i in left_X]))
        # print(leftx, lefty, rightx, righty)

        left_calculated_weight = ((left_X - x_mean) * (left_Y - y_mean)).sum() / ((left_X - x_mean)**2).sum()

        right_X = rightx
        right_Y = righty
        x_mean = np.mean(right_X) 
        y_mean = np.mean(right_Y) 
        right_calculated_weight = ((right_X - x_mean) * (right_Y - y_mean)).sum() / ((right_X - x_mean)**2).sum()

...
def steering_theta(w1, w2):
    if np.abs(w1) > np.abs(w2):  # 우회전
        ...
        if w1 * w2 < 0:  #정방향 or 약간 틀어진 방향
            w1 = -w1
            angle = np.arctan(np.abs(math.tan(w1) - math.tan(w2)) / (1 + math.tan(w1)*math.tan(w2)))
            theta = matching(angle, -np.pi/2, np.pi/2, 0, 90)
        elif w1 * w2 > 0:  #극한으로 틀어진 방향
            if w1 > w2:
                theta = 90
            else:
...
cv2.imwrite("aaa.jpg", result)
w1, w2 = create_weights()
print(w1, w2)
print(steering_theta(w1, w2))
# print(steering())
# steering_theta(w1, w2)
